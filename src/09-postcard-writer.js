...
export function isValidPincode(code) {
  if(typeof code !== "string" || code.startsWith(0)===true || code.length!==6) return false
  const regex = /^\d+$/
...
export function formatPostcardField(label, value, width) {
  ...
  // return ` ${label.padEnd(!width?12:width)} : + ${value} `
  if(!width){
   width = 12
  }
...
export function isFromState(address, stateCode) {
  ...
  if(address.endsWith(stateCode)===true) return true
  return false
}
